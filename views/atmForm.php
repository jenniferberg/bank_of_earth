...
<div class="instructions center">
  ...
  Minimum deposit or withdrawal of $25 is required.
  <br />
...
<table class="formReview">
  <tr>
    <th class="right">Select Account Number:</th>
	<td class="left"><select name="account_no">
	<?php
	$count = count($accounts);
	for($i = 0; $i < $count; $i++){
		echo "<option value=\"{$accounts[$i]['account_no']}\"";
		if($account_no == $accounts[$i]['account_no']){
			echo ' selected';
		}
		echo ">";
  ...
  <tr>
    <th class="right">Type of Transaction:</th>
    <td class="left"><select name="type">
	  <option value="Cash Deposit" <?php echo $type == "Cash Deposit" ? 'selected' : ''; ?>>Cash Deposit</option>
	  <option value="Check Deposit" <?php echo $type == "Check Deposit" ? 'selected' : ''; ?>>Check Deposit</option>
